...
$idsupervisorprofile=mysql_result($result,0,"idprofile");
	...
	//}else{
	//	$sql="SELECT iduser,idsupervisor FROM user WHERE idsupervisor='".$iduser."' ; ";
	//}
...
while($rowUsers = mysql_fetch_array($resultUsers)) {
	...
	$sql_profilecheck="SELECT `idsupervisor` FROM user WHERE `iduser` = '".$rowUsers['iduser']."';";
	error_log('test-fetchPresiedentRecords'.$sql_profilecheck);
	$result_profilecheck=mysql_query($sql_profilecheck);
	$currentUserSupervisor=mysql_result($result_profilecheck,0,"idsupervisor");
	
	if($currentUserSupervisor!=$iduser){
	
		if($workflowProfile!=$idsupervisorprofile){
		continue;
		}
	}
	
	$_SESSION['supervisorRecordCount']=$_SESSION['supervisorRecordCount']+1;

		$records.="<tr>
				<td><input type='checkbox' name='supervisorRecordsCheckbox' /></td><td  class='requestInformation'  class='requestInformation' >";
