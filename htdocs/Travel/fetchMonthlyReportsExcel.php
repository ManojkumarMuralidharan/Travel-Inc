...
while($row = mysql_fetch_assoc($result))
{
		...
		$dateto=explode("-",$row['todate']);
			//if(((in_array($datefrom[0], $months))&&$datefrom[2]==$year)||((in_array($dateto[0], $months))&&$dateto[2]==$year)){
					...
					$objPHPExcel->getActiveSheet()->SetCellValue('B'.$row_count, $row['idexpenserecords']);
					$objPHPExcel->getActiveSheet()->SetCellValue('C'.$row_count, $row['fromdate']);
					$objPHPExcel->getActiveSheet()->SetCellValue('D'.$row_count, $row['todate']);
					...
					$objPHPExcel->getActiveSheet()->SetCellValue('G'.$row_count, $row['idtraveltype']);
